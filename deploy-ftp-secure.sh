...
# FTP Deployment Script for froge.host (Secure Version using .netrc)
# This script uploads your site to froge.host via FTP without storing credentials in the script
...
echo -e "${YELLOW}  (Secure .netrc version)${NC}"
echo -e "${YELLOW}=====================================${NC}"
echo ""
...
if [ ! -f ~/.netrc ]; then
    echo -e "${YELLOW}No ~/.netrc file found. Creating one...${NC}"
    echo ""
    read -p "Enter your FTP password: " -s FTP_PASS
    echo ""
...
EOF
    chmod 600 ~/.netrc
    echo -e "${GREEN}Created ~/.netrc with secure permissions${NC}"
    echo ""
fi
...
# Deploy using lftp (credentials from .netrc)
echo -e "${GREEN}Step 2: Uploading files via FTP...${NC}"

lftp -c "
set ftp:list-options -a;
set ssl:verify-certificate no;
open ftp://$FTP_USER@$FTP_HOST;
lcd $PWD;
cd $REMOTE_DIR;
